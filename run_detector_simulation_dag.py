...
parser.add_argument('--event_lists', nargs='+', default=[],
                    help="If present, scripts read from the given event lists"+
                    " and ignores iterations and energies parameters")
...
if args.event_lists!=[]:
    ...
    zfill_amount = len(str(len(args.event_lists)-1))
...
if args.event_lists!=[]:
    for i, infile in enumerate(args.event_lists):
        transfer_files = []
        file_remaps = []
        if output_index!=-1:
    ...
    for energy in args.energies:
        if "." in energy:
            energy = energy[:energy.index(".")]+energy[energy.index("e"):]
